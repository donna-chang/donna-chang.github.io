      ...
      const fsSource = `
        ...
        void main(){
         ...
         /* 保留原本 shader 後續會使用的 centreBlend */
          ...
          /*
           * Scroll-responsive vertical lens field.
           * As content scrolls behind the fixed header, it passes through
           * different parts of this distortion field.
           */
          float signedY = p_px.y / max(b_px.y, 1.0);
          float verticalPos = clamp(abs(signedY), 0.0, 1.0);

          /* Calm centre, stronger pull near the top / bottom edges. */
          float verticalEdge = smoothstep(0.12, 1.0, verticalPos);
          float verticalCurve = sign(signedY) * pow(verticalEdge, 1.55);

          vec2 offset = vec2(0.0, offsetAmt * verticalCurve);
          ...
          if (u_aberration > 0.0) {
               ...
               * RGB channels use slightly different strengths of the same
               * lens distortion instead of a flat colour halo.
               */
              float chromaStrength = clamp(u_aberration * 8.0, 0.0, 0.32);

              vec2 rUV = mapped + offset * (1.0 + chromaStrength) - tiltOffset;
              vec2 gUV = mapped + offset - tiltOffset;
              vec2 bUV = mapped + offset * (1.0 - chromaStrength) - tiltOffset;

              float rOob = max(max(-rUV.x, rUV.x - 1.0), max(-rUV.y, rUV.y - 1.0));
              float gOob = max(max(-gUV.x, gUV.x - 1.0), max(-gUV.y, gUV.y - 1.0));
              float bOob = max(max(-bUV.x, bUV.x - 1.0), max(-bUV.y, bUV.y - 1.0));

              float rBlend = 1.0 - smoothstep(0.0, 0.01, rOob);
              float gBlend = 1.0 - smoothstep(0.0, 0.01, gOob);
              float bBlend = 1.0 - smoothstep(0.0, 0.01, bOob);

              rUV = mix(mapped, rUV, rBlend);
              gUV = mix(mapped, gUV, gBlend);
              bUV = mix(mapped, bUV, bBlend);

              refrCol.r = texture2D(u_tex, rUV).r;
              refrCol.g = texture2D(u_tex, gUV).g;
          ...
          /* Frosted-white veil inside the WebGL result. */
          float glassHaze = 0.16;
          final.rgb = mix(final.rgb, vec3(0.97, 0.97, 0.97), glassHaze);
