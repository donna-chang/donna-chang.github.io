      ...
      const fsSource = `
        ...
        void main(){
         ...
         /* 保留原本 shader 後續會使用的 centreBlend */
          ...
          /*
           * Vertical lens warp — no diagonal pull.
           *
           * This is a true Y-coordinate remap, not just a fixed Y offset:
           * content is stretched through the middle of the glass and changes
           * shape as it scrolls through the fixed header.
           */
          float lensY = clamp((v_uv.y - 0.5) * 2.0, -1.0, 1.0);
          float absLensY = abs(lensY);

          /* Strongest deformation through the body of the lens, smooth at edges. */
          float stretchProfile = lensY * (1.0 - lensY * lensY);

          /*
           * Use the existing refraction/bevel controls to drive the amount.
           * With the supplied script values this lands around 0.40.
           */
          float stretchStrength = clamp(
            u_refraction * 7.0 + u_bevelDepth * 1.25,
            0.18,
            0.48
          );

          /*
           * Negative sign pulls source samples toward the lens centre.
           * That makes the displayed content expand vertically rather than
           * merely slide up/down.
           */
          float stretchDeltaY =
            -stretchProfile * u_bounds.w * stretchStrength;

          /* A small top/bottom edge pull keeps the liquid-glass rim alive. */
          float verticalEdge = smoothstep(0.58, 1.0, absLensY);
          float edgeDir = lensY < 0.0 ? -1.0 : 1.0;
          float edgePullY = edgeDir * offsetAmt * verticalEdge * 0.55;

          vec2 offset = vec2(0.0, stretchDeltaY + edgePullY);
          ...
          if (u_aberration > 0.0) {
              /*
               * Dispersion follows the SAME vertical lens warp.
               * R/G/B use slightly different warp strengths, so the colour
              ...
              vec2 rWarp = vec2(
                0.0,
                stretchDeltaY * (1.0 + dispersion) +
                edgePullY * (1.0 + dispersion * 0.70)
              );
              ...
              vec2 rUV = mapped + rWarp - tiltOffset;
              vec2 bUV = mapped + bWarp - tiltOffset;

              float rOob = max(max(-rUV.x, rUV.x - 1.0), max(-rUV.y, rUV.y - 1.0));
              float bOob = max(max(-bUV.x, bUV.x - 1.0), max(-bUV.y, bUV.y - 1.0));

              rUV = mix(mapped, rUV, 1.0 - smoothstep(0.0, 0.01, rOob));
              bUV = mix(mapped, bUV, 1.0 - smoothstep(0.0, 0.01, bOob));

              refrCol.r = texture2D(u_tex, rUV).r;
              refrCol.b = texture2D(u_tex, bUV).b;
          }
