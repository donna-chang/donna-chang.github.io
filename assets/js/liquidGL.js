...
(() => {
  ...
  class liquidGLLens {
    constructor(renderer, element, options) {
      ...
      /*
       * Keep LiquidGL's WebGL rendering untouched, but restore a very
       * subtle portion of the target's original background color as a tint.
       * This makes the glass feel slightly less transparent without changing
       * refraction, frost sampling, chromatic aberration, geometry, or input.
       */
      if (this._bgColorComponents) {
        const { r, g, b, a } = this._bgColorComponents;
        const tintAlpha = Math.min(a * 0.6, 0.06);
        this.el.style.backgroundColor = `rgba(${r}, ${g}, ${b}, ${tintAlpha})`;
      }
