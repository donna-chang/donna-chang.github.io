  ...
  class liquidGLRenderer {
    ...
    _initGL() {
      ...
      const fsSource = `
        ...
        /* Smooth fixed blur — avoids random frost grain */
        vec4 smoothBlur5(vec2 uv, vec2 stepSize) {
          vec4 col = texture2D(u_tex, uv) * 4.0;
          col += texture2D(u_tex, uv + vec2( stepSize.x, 0.0));
          col += texture2D(u_tex, uv + vec2(-stepSize.x, 0.0));
          col += texture2D(u_tex, uv + vec2(0.0,  stepSize.y));
          col += texture2D(u_tex, uv + vec2(0.0, -stepSize.y));
          return col / 8.0;
        }
        ...
        void main(){
         ...
         /* 保留原本 shader 後續會使用的 centreBlend */
          ...
          /*
           * Smooth deterministic glass blur.
           * Keeps frost as the blur-strength control, but removes
           * the random sampling that caused grain / color spreading.
           */
          float blurRadius = u_frost > 0.0 ? u_frost * 4.0 : 1.0;
          vec2 blurStep = texel * blurRadius;
          refrCol = smoothBlur5(sampleUV, blurStep);

          if (u_aberration > 0.0) {
              vec2 chroma = offset * u_aberration;

              /* Keep chromatic aberration, but blur R/B consistently too. */
              refrCol.r = smoothBlur5(sampleUV - chroma, blurStep).r;
              refrCol.b = smoothBlur5(sampleUV + chroma, blurStep).b;
          }
